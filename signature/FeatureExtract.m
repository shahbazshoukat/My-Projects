...
%1000 pixels because it is noise
...
%other feature of the signature
stats = regionprops(thresh,'Area','Perimeter','BoundingBox','Eccentricity','Centroid','FilledImage');
...
sig_size = [width,height];

%get short to longer width and height vector to get rid of a few
%orientation problem
[T,I_max] = max(sig_size);
[T,I_min] = min(sig_size);
longer = sig_size(I_max);
shorter = sig_size(I_min);
...
rgbOut = cat(3, channelR, channelG, channelB);
 ...
 title('Segmented out RGB signature with white surrounding')
