...
SnakeLadders::SnakeLadders() :backgroundTexture(),menuBackgroundTexture(), background(),menuBackground(),
...
SnakeLadders::SnakeLadders(int r, int c, int cap) :backgroundTexture(), menuBackgroundTexture(), background(), menuBackground(),
board(), boardSp(), pawnTexture(), diceTexture(), dice(), font()
{
	//window.create(sf::VideoMode(1200, 680), "Snake Ladders || by Shahbaz Shoukat");
...
void SnakeLadders::drawMenu()
{

}
/*void SnakeLadders::runGame()
{
	while (window.isOpen())
	{
		window.clear();
...
{
	backgroundTexture.loadFromFile("F:\\BSSE\\2nd Semester\\snakeLadders\\images\\background.jpg");
	returnTexture.loadFromFile("F:\\BSSE\\2nd Semester\\snakeLadders\\images\\exit.png");
	if (!font.loadFromFile("C:\\Windows\\Fonts\\Arial.ttf"))
	{
		//error in loading
	}
	...
	returnToMenu.setPosition(sf::Vector2f(680, 450));
	sf::Text helpText;
	helpText.setFont(font);
	helpText.setScale(sf::Vector2f(0.85, 0.85));
	helpText.setPosition(sf::Vector2f(680, 50));
